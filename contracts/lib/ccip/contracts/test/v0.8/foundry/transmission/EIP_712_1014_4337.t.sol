...
contract EIP_712_1014_4337 is BaseTest {
    ...
    function testEIP712EIP4337WithExistingSmartContractAccount() public {
        ...
        address toDeployAddress =
            SmartContractAccountHelper.calculateSmartContractAccountAddress(END_USER, ENTRY_POINT, address(factory));

        // Deploy the end-contract.
        bytes32 salt = bytes32(uint256(uint160(END_USER)) << 96);
        bytes memory fullInitializeCode =
            SmartContractAccountHelper.getSCAInitCodeWithConstructor(END_USER, ENTRY_POINT);
        ...
        bytes memory fullEncoding =
            SmartContractAccountHelper.getFullEndTxEncoding(address(greeter), uint256(0), 0, encodedGreetingCall);
    ...
    function testEIP712EIP4337AndCreateSmartContractAccount() public {
        ...
        bytes memory fullInitializeCode =
            SmartContractAccountHelper.getInitCode(address(factory), END_USER, ENTRY_POINT);
    ...
    function testEIP712EIP4337AndCreateSmartContractAccountWithPaymasterForVRFRequest() public {
        ...
        uint256 fee = 1 ether;
        bytes memory encodedVRFRequestCallData =
            bytes.concat(VRFConsumer.doRequestRandomness.selector, abi.encode(keyhash, fee));
        ...
        SCALibrary.DirectFundingData memory directFundingData =
            SCALibrary.DirectFundingData({recipient: address(vrfConsumer), topupThreshold: 1, topupAmount: 10 ether});
