...
contract Aggregator is AggregatorInterface, ChainlinkClient, Ownable {
     ...
     */
    function chainlinkCallback(bytes32 _clRequestId, int256 _response) external {
    ...
    function updateRequestDetails(
        ...
        address[] _oracles,
        bytes32[] _jobIds
    ) public onlyOwner validateAnswerRequirements(_minimumResponses, _oracles, _jobIds) {
     ...
     */
    function transferLINK(address _recipient, uint256 _amount) public onlyOwner {
     ...
     */
    function cancelRequest(bytes32 _requestId, uint256 _payment, uint256 _expiration)
        external
        ensureAuthorizedRequester
    {
        ...
        cancelChainlinkRequest(_requestId, _payment, this.chainlinkCallback.selector, _expiration);
    }
     ...
     */
    function quickselect(int256[] memory _a, uint256 _k) private pure returns (int256) {
        int256[] memory a = _a;
        uint256 k = _k;
     ...
     */
    modifier validateAnswerRequirements(uint256 _minimumResponses, address[] _oracles, bytes32[] _jobIds) {
    ...
    modifier ensureAuthorizedRequester() {
        require(
            authorizedRequesters[msg.sender] || msg.sender == owner, "Not an authorized address for creating requests"
        );
        _;
    }
}
