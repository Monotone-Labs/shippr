...
contract ConcreteChainlinked is Chainlinked {
    ...
    function publicNewRequest(bytes32 _id, address _address, bytes _fulfillmentSignature) public {
    ...
    function publicRequest(bytes32 _id, address _address, bytes _fulfillmentSignature, uint256 _wei) public {
        Chainlink.Request memory req = newRequest(_id, _address, bytes4(keccak256(_fulfillmentSignature)));
    ...
    function publicCancelRequest(bytes32 _requestId, uint256 _payment, bytes4 _callbackFunctionId, uint256 _expiration)
        public
    {
    ...
    function fulfillRequest(bytes32 _requestId, bytes32) public recordChainlinkFulfillment(_requestId) {} // solhint-disable-line no-empty-blocks
