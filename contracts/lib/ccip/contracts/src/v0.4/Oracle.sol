...
contract Oracle is ChainlinkRequestInterface, OracleInterface, Ownable {
    ...
    function oracleRequest(
        ...
        commitments[requestId] =
            keccak256(abi.encodePacked(_payment, _callbackAddress, _callbackFunctionId, expiration));
     ...
     */
    function withdraw(address _recipient, uint256 _amount) external onlyOwner hasAvailableFunds(_amount) {
     ...
     */
    function cancelOracleRequest(bytes32 _requestId, uint256 _payment, bytes4 _callbackFunc, uint256 _expiration)
        external
    {
