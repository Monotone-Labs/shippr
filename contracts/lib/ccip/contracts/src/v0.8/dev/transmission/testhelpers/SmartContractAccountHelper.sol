...
library SmartContractAccountHelper {
    ...
    function getFullEndTxEncoding(address endContract, uint256 value, uint256 deadline, bytes memory data)
        public
        view
    ...
    function getInitCode(address factory, address owner, address entryPoint)
        external
        pure
        returns (bytes memory initCode)
    {
        ...
        initCode = bytes.concat(
            ...
            abi.encodeWithSelector(
                SmartContractAccountFactory.deploySmartContractAccount.selector, salt, initializeCodeWithConstructor
            )
        );
    }

    /// @dev Computes the smart contract address that results from a CREATE2 operation, per EIP-1014.
    function calculateSmartContractAccountAddress(address owner, address entryPoint, address factory)
        external
        pure
        returns (address)
    {
        ...
        return
            address(uint160(uint256(keccak256(abi.encodePacked(hex"ff", address(factory), salt, initializeCodeHash)))));
    }

    function getAbiEncodedDirectRequestData(address recipient, uint256 topupThreshold, uint256 topupAmount)
        external
        view
