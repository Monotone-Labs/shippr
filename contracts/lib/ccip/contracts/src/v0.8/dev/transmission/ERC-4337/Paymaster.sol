...
contract Paymaster is IPaymaster, ConfirmedOwner {
    ...
    constructor(LinkTokenInterface linkToken, AggregatorV3Interface linkEthFeed, address entryPoint)
    ...
    function handleExtraCostJuels(UserOperation calldata userOp) internal returns (uint256 extraCost) {
        ...
        if (paymentType == uint8(SCALibrary.LinkPaymentType.DIRECT_FUNDING)) {
            SCALibrary.DirectFundingData memory directFundingData =
                abi.decode(userOp.paymasterAndData[21:], (SCALibrary.DirectFundingData));
            if (
