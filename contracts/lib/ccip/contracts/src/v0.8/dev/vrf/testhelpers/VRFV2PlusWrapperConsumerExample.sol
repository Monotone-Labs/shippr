...
contract VRFV2PlusWrapperConsumerExample is VRFV2PlusWrapperConsumerBase, ConfirmedOwner {
    ...
    constructor(address _link, address _vrfV2Wrapper)
        ...
        VRFV2PlusWrapperConsumerBase(_link, _vrfV2Wrapper)
    {}

    function makeRequest(uint32 _callbackGasLimit, uint16 _requestConfirmations, uint32 _numWords)
        external
        onlyOwner
        returns (uint256 requestId)
    {
        ...
        s_requests[requestId] =
            RequestStatus({paid: paid, randomWords: new uint256[](0), fulfilled: false, native: false});
        emit WrapperRequestMade(requestId, paid);
        return requestId;
    }

    function makeRequestNative(uint32 _callbackGasLimit, uint16 _requestConfirmations, uint32 _numWords)
        external
        onlyOwner
        returns (uint256 requestId)
    {
        ...
        s_requests[requestId] =
            RequestStatus({paid: paid, randomWords: new uint256[](0), fulfilled: false, native: true});
        emit WrapperRequestMade(requestId, paid);
        return requestId;
    }
