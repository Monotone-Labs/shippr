    ...
    VRFV2PlusWrapperInterface
{
    ...
    function calculateRequestPriceNativeInternal(uint256 _gas, uint256 _requestGasPrice)
        internal
        view
        returns (uint256)
    {
    ...
    function calculateRequestPriceInternal(uint256 _gas, uint256 _requestGasPrice, int256 _weiPerUnitLink)
        internal
        view
        returns (uint256)
    {
        ...
        onlyConfiguredNotDisabled
    {
        ...
        s_callbacks[requestId] =
            Callback({callbackAddress: _sender, callbackGasLimit: callbackGasLimit, requestGasPrice: tx.gasprice});
        lastRequestId = requestId;
    }

    function requestRandomWordsInNative(uint32 _callbackGasLimit, uint16 _requestConfirmations, uint32 _numWords)
        external
        payable
        override
        returns (uint256 requestId)
    {
        ...
        s_callbacks[requestId] =
            Callback({callbackAddress: msg.sender, callbackGasLimit: _callbackGasLimit, requestGasPrice: tx.gasprice});
...
interface ExtendedVRFCoordinatorV2PlusInterface is IVRFCoordinatorV2Plus {
    ...
    function s_feeConfig()
        external
        view
        returns (uint32 fulfillmentFlatFeeLinkPPM, uint32 fulfillmentFlatFeeNativePPM);
}
