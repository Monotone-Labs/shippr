...
contract VRFCoordinatorV2_5 is VRF, SubscriptionAPI, IVRFCoordinatorV2Plus {
    ...
    event RandomWordsFulfilled(
        uint256 indexed requestId, uint256 outputSeed, uint256 indexed subId, uint96 payment, bool success
    );
    ...
    function computeRequestId(bytes32 keyHash, address sender, uint256 subId, uint64 nonce)
        internal
        pure
    ...
    */
        ...
        returns (uint96)
    {
        ...
        // stack too deep error
        {
            ...
            uint96 payment =
                calculatePaymentAmount(startGas, s_config.gasAfterPaymentCalculation, tx.gasprice, nativePayment);
            if (nativePayment) {
    ...
    function calculatePaymentAmount(
        ...
        if (nativePayment) {
            return calculatePaymentAmountNative(
                startGas, gasAfterPaymentCalculation, s_feeConfig.fulfillmentFlatFeeNativePPM, weiPerUnitGas
            );
        }
        return calculatePaymentAmountLink(
            startGas, gasAfterPaymentCalculation, s_feeConfig.fulfillmentFlatFeeLinkPPM, weiPerUnitGas
        );
    }
    ...
    function calculatePaymentAmountLink(
        ...
        uint256 paymentNoFee = (
            1e18 * (weiPerUnitGas * (gasAfterPaymentCalculation + startGas - gasleft()) + l1CostWei)
