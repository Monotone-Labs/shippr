...
contract VRFSubscriptionBalanceMonitor is ConfirmedOwner, Pausable, KeeperCompatibleInterface {
        ...
        for (uint256 idx = 0; idx < watchList.length; idx++) {
            ...
            if (
                target.lastTopUpTimestamp + minWaitPeriod <= block.timestamp
                    && contractBalance >= target.topUpAmountJuels && subscriptionBalance < target.minBalanceJuels
            ) {
        ...
        for (uint256 idx = 0; idx < needsFunding.length; idx++) {
            ...
            if (
                ...
                bool success = LINKTOKEN.transferAndCall(
                    address(COORDINATOR), target.topUpAmountJuels, abi.encode(needsFunding[idx])
                );
                if (success) {
    ...
    function getSubscriptionInfo(uint64 subscriptionId)
        external
        view
        returns (bool isActive, uint96 minBalanceJuels, uint96 topUpAmountJuels, uint56 lastTopUpTimestamp)
    {
