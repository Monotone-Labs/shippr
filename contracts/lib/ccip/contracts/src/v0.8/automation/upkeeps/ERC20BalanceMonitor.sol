...
contract ERC20BalanceMonitor is ConfirmedOwner, Pausable, KeeperCompatibleInterface {
     ...
     */
    constructor(address erc20TokenAddress, address keeperRegistryAddress, uint256 minWaitPeriodSeconds)
     ...
     */
    function setWatchList(address[] calldata addresses, uint96[] calldata minBalances, uint96[] calldata topUpLevels)
        external
        onlyOwner
    {
        if (
    ...
    function getAccountInfo(address targetAddress)
        external
        view
        returns (bool isActive, uint96 minBalance, uint96 topUpLevel, uint56 lastTopUpTimestamp)
    {
