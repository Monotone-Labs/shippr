...
contract CronUpkeepFactory is ConfirmedOwner {
    ...
    function newCronUpkeepWithJob(bytes memory encodedJob) public {
        emit NewCronUpkeepCreated(
            address(new CronUpkeep(msg.sender, s_cronDelegate, s_maxJobs, encodedJob)), msg.sender
        );
    }
     ...
     */
    function encodeCronJob(address target, bytes memory handler, string memory cronString)
        external
        pure
