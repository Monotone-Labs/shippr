...
abstract contract KeeperRegistryBase2_1 is ConfirmedOwner, ExecutionPrevention {
     ...
     */
    constructor(Mode mode, address link, address linkNativeFeed, address fastGasFeed, address automationForwarderLogic)
     ...
     */
    function _getMaxGasOverhead(Trigger triggerType, uint32 performDataLength, uint8 f)
        internal
        pure
        returns (uint256)
    {
     ...
     */
    function _updateTransmitterBalanceFromPool(address transmitterAddress, uint96 totalPremium, uint96 payeeCount)
        internal
        returns (uint96)
    {
    ...
    function _checkPayload(uint256 upkeepId, Trigger triggerType, bytes memory triggerData)
        internal
        view
     ...
     */
    function _prePerformChecks(uint256 upkeepId, bytes memory rawTrigger, UpkeepTransmitInfo memory transmitInfo)
        internal
    ...
    function _validateLogTrigger(uint256 upkeepId, bytes memory rawTrigger, UpkeepTransmitInfo memory transmitInfo)
        internal
        returns (bool, bytes32)
    {
     ...
     */
    function _performUpkeep(IAutomationForwarder forwarder, uint256 performGas, bytes memory performData)
        internal
        nonReentrant
