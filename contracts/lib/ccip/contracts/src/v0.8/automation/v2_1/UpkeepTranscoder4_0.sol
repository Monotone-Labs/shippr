...
contract UpkeepTranscoder4_0 is UpkeepTranscoderInterfaceV2, TypeAndVersionInterface {
     ...
     */
    function transcodeUpkeeps(uint8 fromVersion, uint8, bytes calldata encodedUpkeeps)
        external
        view
        override
        returns (bytes memory)
    {
        ...
        if (fromVersion == uint8(RegistryVersion.V12)) {
            ...
            return abi.encode(
                ids, newUpkeeps, targets, admins, checkDatas, new bytes[](ids.length), new bytes[](ids.length)
            );
        }
        // v1.3 => v2.1
