...
contract KeeperRegistrar2_0 is TypeAndVersionInterface, ConfirmedOwner, IERC677Receiver {
    ...
    event ConfigChanged(
        AutoApproveType autoApproveConfigType, uint32 autoApproveMaxAllowed, address keeperRegistry, uint96 minLINKJuels
    );
    ...
    */
    ...
    function _register(RegistrationParams memory params, address sender) private returns (uint256) {
        ...
        bytes32 hash = keccak256(
            abi.encode(
                params.upkeepContract, params.gasLimit, params.adminAddress, params.checkData, params.offchainConfig
            )
        );
    ...
    modifier isActualAmount(uint256 expected, bytes calldata data) {
        ...
        (,,,,,,, uint96 amount,) =
            abi.decode(data[4:], (string, bytes, address, uint32, address, bytes, bytes, uint96, address));
        if (expected != amount) {
