...
contract VerifiableLoadLogTriggerUpkeep is VerifiableLoadBase, StreamsLookupCompatibleInterface, ILogAutomation {
     ...
     */
    constructor(AutomationRegistrar2_1 _registrar, bool _useArb, bool _useMercury)
        ...
        if (log.topics[0] == sig) {
            ...
            if (useMercury) {
                revert StreamsLookup(
                    feedParamKey, feedsHex, timeParamKey, timeParam, abi.encode(upkeepId, blockNum, addr)
                );
            }
