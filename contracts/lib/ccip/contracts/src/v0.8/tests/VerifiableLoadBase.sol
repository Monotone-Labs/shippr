...
abstract contract VerifiableLoadBase is ConfirmedOwner {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (triggerType == 1) {
                ...
                bytes memory triggerCfg =
                    this.getLogTriggerConfig(address(this), selector, topic0, topic2, topic2, topic3);
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (triggerType == 1) {
                ...
                bytes memory triggerCfg = this.getLogTriggerConfig(
                    address(this), selector, sig, bytes32(abi.encode(upkeepId)), bytes32(0), bytes32(0)
                );
                registry.setUpkeepTriggerConfig(upkeepId, triggerCfg);
            }
        }
    ...
    function updateLogTriggerConfig1(
        ...
        registry.setUpkeepTriggerConfig(
            upkeepId, this.getLogTriggerConfig(addr, selector, topic0, topic1, topic2, topic3)
        );
    }
