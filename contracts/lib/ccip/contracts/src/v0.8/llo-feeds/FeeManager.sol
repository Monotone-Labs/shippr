...
contract FeeManager is IFeeManager, ConfirmedOwner, TypeAndVersionInterface {
     ...
     */
    constructor(address _linkAddress, address _nativeAddress, address _proxyAddress, address _rewardManagerAddress)
    ...
    /// @inheritdoc IFeeManager
    function updateSubscriberDiscount(address subscriber, bytes32 feedId, address token, uint64 discount)
        external
        onlyOwner
    {
    ...
    function _processFee(bytes calldata payload, bytes calldata parameterPayload, address subscriber)
        internal
        view
        returns (Common.Asset memory, Common.Asset memory, uint256)
    {
