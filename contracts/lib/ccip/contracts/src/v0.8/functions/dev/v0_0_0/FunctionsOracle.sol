    ...
    AuthorizedOriginReceiverUpgradeable
{
     ...
     * @inheritdoc IFunctionsOracle
     */
    function getRequiredFee(bytes calldata, /* data */ IFunctionsBillingRegistry.RequestBilling memory /* billing */ )
        public
        pure
        override
        returns (uint96)
        ...
        returns (uint96)
    {
        IFunctionsBillingRegistry.RequestBilling memory billing =
            IFunctionsBillingRegistry.RequestBilling(subscriptionId, msg.sender, gasLimit, gasPrice);
        ...
        returns (bytes32)
    {
        ...
        bytes32 requestId = s_registry.startBilling(
            data, IFunctionsBillingRegistry.RequestBilling(subscriptionId, msg.sender, gasLimit, tx.gasprice)
        );
        emit OracleRequest(
            requestId, msg.sender, tx.origin, subscriptionId, s_registry.getSubscriptionOwner(subscriptionId), data
        );
        return requestId;
    }
