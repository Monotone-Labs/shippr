...
abstract contract FunctionsBilling is Routable, IFunctionsBilling {
    ...
    /// @inheritdoc IFunctionsBilling
    function estimateCost(uint64 subscriptionId, bytes calldata data, uint32 callbackGasLimit, uint256 gasPriceWei)
        external
        view
        override
        returns (uint96)
    ...
    function _calculateCostEstimate(uint32 callbackGasLimit, uint256 gasPriceWei, uint72 donFee, uint72 adminFee)
        internal
        view
        returns (uint96)
    {
    ...
    function _startBilling(FunctionsResponse.RequestMeta memory request)
        internal
    ...
    function _computeRequestId(address don, address client, uint64 subscriptionId, uint64 nonce)
        private
        pure
        returns (bytes32)
    {
