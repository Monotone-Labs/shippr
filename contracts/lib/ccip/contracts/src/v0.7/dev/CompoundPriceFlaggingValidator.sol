...
contract CompoundPriceFlaggingValidator is ConfirmedOwner, KeeperCompatibleInterface {
     ...
     */
    function getFeedDetails(address aggregator) public view returns (string memory, uint8, uint32) {
    ...
    function _isInvalid(address aggregator) private view returns (bool invalid) {
        ...
        (aggregatorPrice, compPrice) =
            _adjustPriceDecimals(aggregatorPrice, aggregatorDecimals, compPrice, compDetails.decimals);
     ...
     */
    function _deviatesBeyondThreshold(uint256 aggregatorPrice, uint256 compPrice, uint32 deviationThresholdNumerator)
        private
        pure
