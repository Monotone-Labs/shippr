...
contract FluxAggregator is AggregatorV2V3Interface, Owned {
    ...
    function updateFutureRounds(
        ...
        emit RoundDetailsUpdated(paymentAmount, _minSubmissions, _maxSubmissions, _restartDelay, _timeout);
    }
    ...
    function initializeNewRound(uint32 _roundId) private {
        ...
        RoundDetails memory nextDetails =
            RoundDetails(new int256[](0), maxSubmissionCount, minSubmissionCount, timeout, paymentAmount);
    ...
    function validateAnswer(uint32 _roundId, int256 _newAnswer) private {
        ...
        try av.validate{gas: VALIDATOR_GAS_LIMIT}(prevAnswerRoundId, prevRoundAnswer, _roundId, _newAnswer) {} catch {}
    }
    ...
    function addOracle(address _oracle, address _admin) private {
        ...
        require(
            oracles[_oracle].admin == address(0) || oracles[_oracle].admin == _admin, "owner cannot overwrite admin"
        );
    ...
    function validateOracleRound(address _oracle, uint32 _roundId) private view returns (bytes memory) {
        ...
        if (_roundId != rrId && _roundId != rrId.add(1) && !previousAndCurrentUnanswered(_roundId, rrId)) {
