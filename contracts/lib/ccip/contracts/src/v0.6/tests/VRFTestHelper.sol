...
contract VRFTestHelper is VRF {
    ...
    function projectiveECAdd_(uint256 px, uint256 py, uint256 qx, uint256 qy)
        public
        pure
    ...
    function affineECAdd_(uint256[2] memory p1, uint256[2] memory p2, uint256 invZ)
        public
        pure
    ...
    function verifyLinearCombinationWithGenerator_(uint256 c, uint256[2] memory p, uint256 s, address lcWitness)
        public
        pure
        returns (bool)
    {
    ...
    function randomValueFromVRFProof_(bytes memory proof) public view returns (uint256 output) {
