...
contract FluxAggregatorTestHelper {
    ...
    function readGetRoundData(address _aggregator, uint80 _roundID) external {
        FluxAggregator(_aggregator).getRoundData(_roundID);
    }

    function readLatestRoundData(address _aggregator) external {
        FluxAggregator(_aggregator).latestRoundData();
    }

    function readLatestAnswer(address _aggregator) external {
        FluxAggregator(_aggregator).latestAnswer();
    }

    function readLatestTimestamp(address _aggregator) external {
        FluxAggregator(_aggregator).latestTimestamp();
    }

    function readLatestRound(address _aggregator) external {
        FluxAggregator(_aggregator).latestRound();
    }

    function requestNewRound(address _aggregator) external {
        requestedRoundId = FluxAggregator(_aggregator).requestNewRound();
    }

    function readGetAnswer(address _aggregator, uint256 _roundID) external {
        FluxAggregator(_aggregator).getAnswer(_roundID);
    }

    function readGetTimestamp(address _aggregator, uint256 _roundID) external {
