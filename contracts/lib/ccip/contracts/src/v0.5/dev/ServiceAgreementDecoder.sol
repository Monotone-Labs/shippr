...
contract ServiceAgreementDecoder {
    ...
    function decodeServiceAgreement(bytes memory _serviceAgreementData)
        internal
        pure
        returns (ServiceAgreement memory)
    {
        ...
        ) = abi.decode(_serviceAgreementData, (uint256, uint256, uint256, address[], bytes32, address, bytes4, bytes4));
