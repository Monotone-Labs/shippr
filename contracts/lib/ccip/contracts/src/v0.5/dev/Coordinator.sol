...
contract Coordinator is
    ...
    ServiceAgreementDecoder,
    OracleSignaturesDecoder
{
    ...
    event NewServiceAgreement(bytes32 indexed said, bytes32 indexed requestDigest);

    event CancelOracleRequest(bytes32 internalId);
    ...
    ) external onlyLINK sufficientLINK(_amount, _sAId) checkCallbackAddress(_callbackAddress) 
     ...
     */
    function initiateServiceAgreement(bytes memory _serviceAgreementData, bytes memory _oracleSignaturesData)
        public
        returns (bytes32 serviceAgreementID)
    {
        ...
        require(
            _agreement.oracles.length == _signatures.vs.length && _signatures.vs.length == _signatures.rs.length
                && _signatures.rs.length == _signatures.ss.length,
            "Must pass in as many signatures as oracles"
        );
        // solhint-disable-next-line not-rely-on-time
        require(_agreement.endAt > block.timestamp, "ServiceAgreement must end in the future");
        require(serviceAgreements[serviceAgreementID].endAt == 0, "serviceAgreement already initiated");
        serviceAgreementID = getId(_agreement);

        registerOracleSignatures(serviceAgreementID, _agreement.oracles, _signatures);
        ...
        (bool ok, bytes memory response) = _agreement.aggregator.call(
            abi.encodeWithSelector(_agreement.aggInitiateJobSelector, serviceAgreementID, _serviceAgreementData)
        );
    ...
    function registerOracleSignatures(
        ...
        for (uint256 i = 0; i < _oracles.length; i++) {
            address signer = getOracleAddressFromSASignature(
                _serviceAgreementID, _signatures.vs[i], _signatures.rs[i], _signatures.ss[i]
            );
     ...
     */
    function getOracleAddressFromSASignature(bytes32 _serviceAgreementID, uint8 _v, bytes32 _r, bytes32 _s)
        private
        pure
        returns (address)
    {
        ...
        returns (bool)
    {
        ...
        (bool ok, bytes memory aggResponse) = sA.aggregator.call(
            abi.encodeWithSelector(sA.aggFulfillSelector, _requestId, callback.sAId, msg.sender, _data)
        );
        ...
        if (aggComplete) {
            ...
            for (uint256 oIdx = 0; oIdx < sA.oracles.length; oIdx++) {
                // pay oracles
                withdrawableTokens[sA.oracles[oIdx]] =
                    uint256(int256(withdrawableTokens[sA.oracles[oIdx]]) + paymentAmounts[oIdx]);
            } // solhint-disable-next-line avoid-low-level-calls
            (bool success,) = callback.addr.call(
                abi.encodeWithSelector( // report final result
     ...
     */
    function cancelOracleRequest(bytes32, uint256, bytes4, uint256) external {} // solhint-disable-line no-empty-blocks
    ...
    modifier checkServiceAgreementPresence(bytes32 _sAId) {
        require(uint256(serviceAgreements[_sAId].requestDigest) != 0, "Must reference an existing ServiceAgreement");
        _;
    }
