...
contract MaliciousChainlinkClient is ChainlinkClient {
        ...
        returns (bytes32 requestId)
    {
        ...
        require(
            _link.transferAndCall(chainlinkOracleAddress(), _amount, encodeTargetRequest(_req)),
        ...
        returns (bytes32 requestId)
    {
        ...
        LinkTokenInterface _link = LinkTokenInterface(chainlinkTokenAddress());
        require(
            _link.transferAndCall(chainlinkOracleAddress(), _amount, encodePriceRequest(_req)),
            "Unable to transferAndCall to oracle"
        );
        maliciousRequests += 1;
        ...
        returns (bytes32 requestId)
    {
        ...
        LinkTokenInterface _link = LinkTokenInterface(chainlinkTokenAddress());
        require(
            _link.transferAndCall(chainlinkOracleAddress(), _wei, encodeWithdrawRequest(_req)),
            "Unable to transferAndCall to oracle"
        );
        maliciousRequests += 1;
        return requestId;
