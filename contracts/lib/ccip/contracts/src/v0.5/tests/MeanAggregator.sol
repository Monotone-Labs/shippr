...
contract MeanAggregator is ServiceAgreementDecoder {
    ...
    function initiateJob(bytes32 _sAId, bytes memory _serviceAgreementData)
        public
    ...
    function fulfill(bytes32 _requestId, bytes32 _sAId, address _oracle, bytes32 _value)
        public
        returns (bool success, bool complete, bytes memory response, int256[] memory paymentAmounts)
    {
        if (reportingOrder[_requestId][_oracle] != 0 || numberReported[_requestId] == oracles[_sAId].length) {
    ...
    function calculatePayments(bytes32 _sAId, bytes32 _requestId) private returns (int256[] memory paymentAmounts) {
        ...
        for (uint256 oIdx = 0; oIdx < oracles[_sAId].length; oIdx++) {
            ...
            paymentAmounts[oIdx] = int256(
                2 * (totalPayment / numOracles) - ((totalPayment * ((2 * reportingIdx) + 1)) / (numOracles ** 2))
            );
